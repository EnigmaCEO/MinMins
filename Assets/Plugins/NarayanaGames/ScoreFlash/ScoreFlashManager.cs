...
public class ScoreFlashManager : MonoBehaviour {
    ...
    void Awake() {
        if (instance != null && instance != this) {
            ...
            foreach (Transform child in this.transform) {
                Instance.scoreFlashInstances[child.name] = child.GetComponent<ScoreFlash>();
            }
        }
        if (ensureSingleton && !Application.isEditor) {
    ...
    /// <summary>
    ///     Immediately clears all messages of all score flash instances. This
    ///     is often useful when you enter pause, or cancel a session, to make
    ///     sure no messages keep dangling in the UI.
    /// </summary>
    public static void CleanupAllMessages() {
        foreach (IScoreFlash scoreFlashInstance in Instance.scoreFlashInstances.Values) {
            scoreFlashInstance.Cleanup();
        }
    }
