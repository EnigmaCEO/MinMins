...
public class ScoreFlash : ScoreFlashBase, IScoreFlash, IScoreFlashLayout {
    ...
    // used to create a state where we always do assign parents; for cleaner scenes (during warmup)
    private bool forWarmUp = false;
    ...
    /// <summary>
    ///     Set this to true if you want to enforce a single message queue. This
    ///     can be useful if you want to use ScoreFlashFollow3D but still want
    ///     to limit the number of messages shown at any given time.
    /// </summary>
    public bool forceSingleMessageQueue = false;
    ...
    ///     that should be <c>true</c>.
    /// </summary>
    public bool spreadImmediately = true;

    /// <summary>
    ///     If <c>true</c>, immediately replaces the previous message.
    /// </summary>
    public bool replacePreviousMessage = false;
    ...
    private List<int> scoreMessageKeys = new List<int>();
    ...
    void Awake() {
        ...
        //Debug.Log("ScoreFlash.Awake() called");
        ...
        if (Application.isPlaying && warmUpCount > 0 && rendering == RenderingType.CustomRenderer) {
            ...
            //Debug.LogFormat(this, "Creating {0} warmup messages ({1})", warmUpCount, this.name);
            forWarmUp = true;
            for (int i = 0; i < warmUpCount; i++) {
                msg = PushLocal(".");
                msg.MakeWarmUpMessage();
                //Debug.LogFormat(msg.scoreFlashRenderer, "Created warmup message {0}/{1} for {2}", i, warmUpCount, this.name);
            }
            forWarmUp = false;
    ...
    private ScoreFlashRendererBase GetRendererFromPool() {
        ...
        if (rendererPool.Count > 0) {
            ...
            //Debug.LogFormat("Got from pool: {0}", objectFromPool.name);
        }
        return objectFromPool;
    }

    private void PushRendererToPool(ScoreFlashRendererBase objectForPool) {
        if (objectForPool.Cleanup()) {
            //Debug.LogFormat("Pushed {0} to pool", objectForPool.name);
            objectForPool.gameObject.SetActive(false);
            if (rendererPool.Contains(objectForPool)) {
                Debug.LogErrorFormat(objectForPool, "Putting {0} to pool even though it already is in pool!", objectForPool.name);
                Debug.Break();
            }
    ...
    internal ScoreMessage PushLocal(object message,
        ...
        //Debug.Log("ScoreFlash.PushLocal() called");
        ...
        if (msg.scoreFlashRenderer == null) {
            ...
            if (!scoreFlashRenderer.NeverAssignParent || forWarmUp) {
    ...
    private bool NeedsGUIStyle {
        get {
            return rendering == RenderingType.UnityGUI_GUISkin 
                || rendering == RenderingType.UnityGUI_Font
                || (rendering == RenderingType.CustomRenderer && scoreFlashRenderer is IHasOnGUI);
        }
    }
    ...
    void FixedUpdate() {
        for (int i=0; i < allFollow3Ds.Count; i++) {
            ScoreFlashFollow3DLocation follow3D = allFollow3Ds[i];
    ...
    public void Cleanup() {
        ...
        for (int i=0; i < scoreMessageKeys.Count; i++) {
            int messageQueueID = scoreMessageKeys[i];
            messagesCount += scoreMessages[messageQueueID].Count;
            while (scoreMessages[messageQueueID].Count > 0) {
                CleanupMessage(messageQueueID);
            }
        }

        //Debug.LogFormat(this, "Cleaned up all {0} messages in {1} queues on {2}", messagesCount, scoreMessageKeys.Count, this.name);

        for (int i = 0; i < completedMessagesQueues.Count; i++) {
            int messageQueueID = completedMessagesQueues[i];
            scoreMessages.Remove(messageQueueID);
            scoreMessageKeys.Remove(messageQueueID);
        }

        // make sure that there are no lingering pooled objects:

        ScoreFlashRendererBase[] poolChildren = GetComponentsInChildren<ScoreFlashRendererBase>();
        for (int i=0; i < poolChildren.Length; i++) {
            if (!rendererPool.Contains(poolChildren[i])) {
                Debug.LogErrorFormat(poolChildren[i], "Had to pull {0} back into pool!", poolChildren[i].name);
                PushRendererToPool(poolChildren[i]);
            }
        }
    }
    ...
    private void AnimateMessages() {
        // animate all currently active messages
        completedMessagesQueues.Clear();
        for (int mqi = 0; mqi < scoreMessageKeys.Count; mqi++) {
            int messageQueueID = scoreMessageKeys[mqi];
            //for (int idx = 0; idx < scoreMessages.Keys.Count; idx++) {
            //    int messageQueueID = scoreMessages.Keys[idx];
            ...
            for (int i = messages.Count - 1; i >= 0; i--) { // oldest one last...
                ...
                if (msg.LocalTimeScale >= 0.1F) {
                    for (int k = i + 1; k < messages.Count; k++) {
                        ...
                        if (replacePreviousMessage) {
                            otherMsg.AddDeltaTimeToAge(10000);
                        }
                    }
                }
            ...
            CleanupOldMessages(messageQueueID);
    ...
    private void CleanupMessage(int messageQueueID) {
        ...
        CleanupMessage(messageQueueID, msg);
    ...
    private void CleanupOldMessages(int messageQueueID) {
        indicesToRemove.Clear();
        NGQueue<ScoreMessage> allMessages = scoreMessages[messageQueueID];
        for (int i = 0; i < allMessages.Count; i++) {
            ScoreMessage msg = allMessages[i];
            float addToRead = CalcAddToRead(msg);
            if (msg.Age > fadeInTimeSeconds + readTimeSeconds + addToRead + fadeOutTimeSeconds + 0.5F) {
                CleanupMessage(messageQueueID, msg);
                indicesToRemove.Add(i);
            }
        }
        for (int i = indicesToRemove.Count - 1; i >= 0; i--) {
            allMessages.RemoveAt(indicesToRemove[i]);
        }
        if (scoreMessages[messageQueueID].Count == 0) {
            completedMessagesQueues.Add(messageQueueID);
        }
    ...
    private void CleanupMessage(int messageQueueID, ScoreMessage msg) {
        if (msg.FollowLocation != null && !allFollow3Ds.Contains(msg.FollowLocation)) {
            allFollow3Ds.Remove(msg.FollowLocation);
        }
        if (Application.isPlaying) {
            //Debug.Log("ScoreFlash.CleanupMessage() called");
    ...
    private void DrawEditorStatistics() {
        ...
        for (int i = 0; i < scoreMessageKeys.Count; i++) {
            int messageQueueID = scoreMessageKeys[i];
            y += 15;
            GUI.Label(new Rect(10, 10 + y, 400, 20), string.Format("Queue {0}: {1} messages", messageQueueID, scoreMessages[messageQueueID].Count));
        }
    }
    ...
    private void RenderScoreMessage() {
        var enumerator = scoreMessages.Keys.GetEnumerator();
        while (enumerator.MoveNext()) {
            int messageQueueID = enumerator.Current;
